...
const props = defineProps({ depots: Object, filters: Object })
...
function applyFilters() {
  ...
  debounce = setTimeout(() => {
    router.get(route('app.depots.index', { _tenant: route().params._tenant }), {
...
function destroy(id) {
  ...
  router.delete(route('app.depots.destroy', { depot: id, _tenant: route().params._tenant }))
}
</script>
...
<template>
  <AppLayout title="Dépôts">
    ...
    <div class="page-header">
      <h1 class="page-title"><i class="fa-solid fa-warehouse" style="color:#6366F1"></i> Dépôts</h1>
      <Link :href="route('app.depots.create', { _tenant: route().params._tenant })" class="btn-primary">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="depots.data.length === 0">
            <td colspan="4" class="empty-row">Aucun dépôt trouvé.</td>
          </tr>
          <tr v-for="d in depots.data" :key="d.id">
            <td class="cell-name">{{ d.name }}</td>
            <td>{{ d.address || '—' }}</td>
            <td>
              <span v-if="d.is_default" class="default-badge"><i class="fa-solid fa-star"></i> Principal</span>
              <span v-else>—</span>
            </td>
            <td class="actions-cell">
              <Link :href="route('app.depots.edit', { depot: d.id, _tenant: route().params._tenant })" class="btn-icon" title="Modifier">
    ...
    <div v-if="depots.last_page > 1" class="pagination">
      <template v-for="link in depots.links" :key="link.label">
...
.btn-primary { background: #6366F1; color: #fff; padding: 8px 16px; font-size: 0.82rem; font-weight: 600; border: none; cursor: pointer; display: flex; align-items: center; gap: 6px; text-decoration: none; }
...
.filter-input:focus { border-color: #6366F1; box-shadow: 0 0 0 2px rgba(99,102,241,0.12); }
...
.default-badge { font-size: 0.72rem; font-weight: 600; padding: 2px 8px; color: #F59E0B; background: #FEF3C7; }
