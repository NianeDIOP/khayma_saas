...
function applyFilters() {
  ...
  debounce = setTimeout(() => {
    router.get(route('app.expense-categories.index', { _tenant: route().params._tenant }), {
...
function destroy(id) {
  ...
  router.delete(route('app.expense-categories.destroy', { expense_category: id, _tenant: route().params._tenant }))
}
</script>
...
<template>
  <AppLayout title="Catégories de dépenses">
    ...
    <div class="page-header">
      <h1 class="page-title"><i class="fa-solid fa-tags" style="color:#DC2626"></i> Catégories de dépenses</h1>
      <Link :href="route('app.expense-categories.create', { _tenant: route().params._tenant })" class="btn-primary">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="categories.data.length === 0">
            <td colspan="3" class="empty-row">Aucune catégorie trouvée.</td>
          </tr>
          <tr v-for="c in categories.data" :key="c.id">
            <td class="cell-name">{{ c.name }}</td>
            <td><span class="count-badge">{{ c.expenses_count }}</span></td>
            <td class="actions-cell">
              <Link :href="route('app.expense-categories.edit', { expense_category: c.id, _tenant: route().params._tenant })" class="btn-icon" title="Modifier">
...
.count-badge { background: #FEE2E2; color: #DC2626; font-size: 0.72rem; font-weight: 600; padding: 2px 8px; }
