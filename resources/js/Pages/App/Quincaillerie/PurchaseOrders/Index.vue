...
function applyFilters() {
  ...
  debounce = setTimeout(() => {
    router.get(route('app.quincaillerie.purchase-orders.index', { _tenant: t() }), {
...
<template>
  <AppLayout title="Bons de commande">
    ...
    <div class="page-header">
      <h1 class="page-title"><i class="fa-solid fa-truck" style="color:#F59E0B"></i> Bons de commande</h1>
      <Link :href="route('app.quincaillerie.purchase-orders.create', { _tenant: t() })" class="btn-primary">
        <i class="fa-solid fa-plus"></i> Nouveau BC
      </Link>
    </div>
    ...
    <div class="filters-bar">
      <div class="filter-input-wrap">
        ...
        <input v-model="search" type="text" placeholder="Réf. ou fournisseur…" class="filter-input" />
      </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="po in purchaseOrders.data" :key="po.id">
            <td class="font-medium">{{ po.reference }}</td>
            <td>{{ po.supplier?.name || '—' }}</td>
            <td class="font-medium">{{ fmt(po.total) }}</td>
            <td><span class="badge" :style="{ background: STATUS_COLORS[po.status] }">{{ STATUS_LABELS[po.status] }}</span></td>
            <td>{{ fmtDt(po.expected_date) }}</td>
            <td>{{ fmtDt(po.created_at) }}</td>
            <td>
              <Link :href="route('app.quincaillerie.purchase-orders.show', { _tenant: t(), purchase_order: po.id })" class="btn-sm">
                <i class="fa-solid fa-eye"></i>
              </Link>
            </td>
          </tr>
          <tr v-if="!purchaseOrders.data.length">
            <td colspan="7" class="empty-row">Aucun bon de commande</td>
          </tr>
        </tbody>
      </table>
    </div>

    <div v-if="purchaseOrders.last_page > 1" class="pagination-wrap">
      <Link v-for="link in purchaseOrders.links" :key="link.label"
            :href="link.url || '#'"
