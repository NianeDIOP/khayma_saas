...
function applyFilters() {
  ...
  debounce = setTimeout(() => {
    router.get(route('app.categories.index', { _tenant: route().params._tenant }), {
...
function destroy(id) {
  ...
  router.delete(route('app.categories.destroy', { category: id, _tenant: route().params._tenant }))
}
</script>
...
<template>
  <AppLayout title="Catégories">
    ...
    <div class="page-header">
      <h1 class="page-title"><i class="fa-solid fa-tags" style="color:#8B5CF6"></i> Catégories</h1>
      <Link :href="route('app.categories.create', { _tenant: route().params._tenant })" class="btn-primary">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="categories.data.length === 0">
            <td colspan="4" class="empty-row">Aucune catégorie trouvée.</td>
          </tr>
          <tr v-for="c in categories.data" :key="c.id">
            <td class="cell-name">{{ c.name }}</td>
            <td>{{ c.parent?.name || '—' }}</td>
            <td><span class="module-badge">{{ c.module }}</span></td>
            <td class="actions-cell">
              <Link :href="route('app.categories.edit', { category: c.id, _tenant: route().params._tenant })" class="btn-icon" title="Modifier">
    ...
    <div v-if="categories.last_page > 1" class="pagination">
...
.btn-primary { background: #8B5CF6; color: #fff; padding: 8px 16px; font-size: 0.82rem; font-weight: 600; border: none; cursor: pointer; display: flex; align-items: center; gap: 6px; text-decoration: none; }
...
.filter-input:focus { border-color: #8B5CF6; box-shadow: 0 0 0 2px rgba(139,92,246,0.12); }
...
.module-badge { font-size: 0.72rem; font-weight: 600; padding: 2px 8px; color: #6B7280; background: #F3F4F6; text-transform: uppercase; }
...
.page-link.active { background: #8B5CF6; color: #fff; border-color: #8B5CF6; }
