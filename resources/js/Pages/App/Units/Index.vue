...
function applyFilters() {
  ...
  debounce = setTimeout(() => {
    router.get(route('app.units.index', { _tenant: route().params._tenant }), {
...
function destroy(id) {
  ...
  router.delete(route('app.units.destroy', { unit: id, _tenant: route().params._tenant }))
}
</script>

<template>
  <AppLayout title="Unités">
    <Head title="Unités" />

    <div class="page-header">
      <h1 class="page-title"><i class="fa-solid fa-ruler" style="color:#0891B2"></i> Unités de mesure</h1>
      <Link :href="route('app.units.create', { _tenant: route().params._tenant })" class="btn-primary">
        <i class="fa-solid fa-plus"></i> Nouvelle unité
      </Link>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="units.data.length === 0">
            <td colspan="5" class="empty-row">Aucune unité trouvée.</td>
          </tr>
          <tr v-for="u in units.data" :key="u.id">
            <td class="cell-name">{{ u.name }}</td>
            <td><span class="symbol-badge">{{ u.symbol }}</span></td>
            <td>{{ u.base_unit?.name || '—' }}</td>
            <td>{{ u.conversion_factor }}</td>
            <td class="actions-cell">
              <Link :href="route('app.units.edit', { unit: u.id, _tenant: route().params._tenant })" class="btn-icon" title="Modifier">
                <i class="fa-solid fa-pen"></i>
              </Link>
              <button @click="destroy(u.id)" class="btn-icon btn-danger" title="Supprimer">
    ...
    <div v-if="units.last_page > 1" class="pagination">
...
.btn-primary { background: #0891B2; color: #fff; padding: 8px 16px; font-size: 0.82rem; font-weight: 600; border: none; cursor: pointer; display: flex; align-items: center; gap: 6px; text-decoration: none; }
...
.filter-input:focus { border-color: #0891B2; box-shadow: 0 0 0 2px rgba(8,145,178,0.12); }
...
.symbol-badge { font-size: 0.75rem; font-weight: 700; padding: 2px 8px; color: #0891B2; background: #ECFEFF; }
...
.page-link.active { background: #0891B2; color: #fff; border-color: #0891B2; }
